"""Shadow Identity OIDC client and opaque browser sessions."""
...
SESSION_COOKIE = "__Host-shadow_garden_session"
OIDC_SCOPES = "openid profile email groups"
ALLOWED_JWT_ALGORITHMS = ("RS256", "RS384", "RS512", "ES256", "ES384", "ES512")

class OIDCError(ValueError):
    """Authentication failure safe to classify without exposing sensitive values."""
...
class OIDCConfig:
    ...
    def validate(self) -> None:
        ...
        if self.client_id != "shadow-garden":
...
class SessionStore:
    """Persistent one-time login transactions, identities, and revocable sessions."""
...
class OIDCClient:
    ...
    def exchange_code(self, *, code: str, verifier: str) -> dict[str, Any]:
        ...
        if not client_secret:
            raise OIDCError("OIDC client secret is unavailable", reason="configuration")
        try:
    ...
    def verify_id_token(self, id_token: str, *, nonce: str) -> dict[str, Any]:
        try:
            ...
            raise OIDCError("ID Token validation failed", reason=reason) from exc
        ...
        if time.time() - float(claims["iat"]) > (
            ...
            raise OIDCError("ID Token issued-at time is too old", reason="issued_at_too_old")
    ...
    def complete_profile_claims(
        ...
        if not secrets.compare_digest(
            ...
            raise OIDCError("OIDC UserInfo subject mismatch", reason="userinfo_subject")
    ...
    def fetch_userinfo(self, access_token: str) -> dict[str, Any]:
        ...
        if not endpoint:
            raise OIDCError("OIDC UserInfo endpoint is unavailable", reason="userinfo")
        try:
        ...
        if not isinstance(payload, dict) or not str(payload.get("sub") or "").strip():
            raise OIDCError("OIDC UserInfo response is invalid", reason="userinfo")
...
def sanitize_return_to(value: str | None) -> str:
    value = str(value or "/admin/").strip()
...
def _require_https_url(label: str, value: str) -> None:
    parsed = urlsplit(value)
    allow_http = os.environ.get("GARDEN_OIDC_ALLOW_HTTP_FOR_TESTS", "").lower() in {
        "1",
        "true",
        "yes",
    }
